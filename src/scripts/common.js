...
var SETTINGS_INTERVAL_RANGE_DEFAULT = Days.days7;
...
function converTimeToSummaryTable(time){
    var timeV = time.split(':');
    var hour  = timeV[0];
    var min   = timeV[1];
    var resultTimeTable = 0;
    if(hour > 0) resultTimeTable = hour*3600;
    resultTimeTable = resultTimeTable + 60*min;
